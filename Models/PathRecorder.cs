...
public class PathRecorder {
    ...
    public List<string> CheckpointNames = new();
    public List<string> CheckpointAbbreviations = new();

    public HashSet<Vector2> CheckpointsVisited { get; set; } = new();
    ...
    public void AddCheckpoint(Checkpoint cp, string name) {
        ...
        if (Checkpoints.Count != 0) {
            ...
            if (cp != null && CheckpointsVisited.Contains(cp.Position))
                return;
            if (cp != null) CheckpointsVisited.Add(cp.Position);
