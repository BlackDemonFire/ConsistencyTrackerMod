...
public class ConsistencyTrackerModule : EverestModule {
    ...
    private readonly HashSet<string> _chaptersThisSession = new();
    ...
    private void Level_OnLoadLevel(Level level, Player.IntroTypes playerIntro, bool isFromLoader) {
        ...
        Logging.Log($"level.Session.LevelData.Name={newCurrentRoom}, playerIntro={playerIntro} | CurrentRoomName: '{_currentRoomName}', PreviousRoomName: '{_previousRoomName}'");
    ...
    private void Level_OnTransitionTo(Level level, LevelData levelDataNext, Vector2 direction) {
        ...
        Logging.Log($"levelData.Name->{roomName}, level.Completed->{level.Completed}, level.NewLevel->{level.NewLevel}, level.Session.StartCheckpoint->{level.Session.StartCheckpoint}");
        var holdingGolden = PlayerIsHoldingGoldenBerry(level.Tracker.GetEntity<Player>());
    ...
    private void ChangeChapter(Session session) {
        ...
        Logging.Log($"Level->{session.Level}, session.Area.GetSID()->{session.Area.GetSID()}, session.Area.Mode->{session.Area.Mode}, chapterNameClean->{chapNameClean}, campaignName->{campaignName}");
    ...
    public void SetNewRoom(string newRoomName, bool countDeath = true, bool holdingGolden = false) {
        ...
        if (_previousRoomName == newRoomName && !_currentRoomCompleted) { //Don't complete if entering previous room and current room was not completed
    ...
    private static bool PlayerIsHoldingGoldenBerry(Player player) {
        ...
        return player.Leader.Followers.Any((f) => {
            if (f.Entity is not Strawberry berry)
                return false;
    ...
    public PathInfo GetPathInputInfo() {
        ...
        if (File.Exists(path)) { //Parse File
            ...
            try {
                ...
                Logging.Log($"\tCouldn't read old path info, created new PathInfo. Old path info content:\n{content}");
    ...
    public ChapterStats GetCurrentChapterStats() {
        ...
        Logging.Log($"CurrentChapterName: '{_currentChapterDebugName}', hasEnteredThisSession: '{hasEnteredThisSession}', ChaptersThisSession: '{string.Join(", ", _chaptersThisSession)}'");
    ...
    public void CreateChapterSummary(int attemptCount) {
        ...
        if (!hasPathInfo) {
            Logging.Log("Called CreateChapterSummary without chapter path info. Please create a path before using this feature");
    ...
    public void WipeChapterData() {
        ...
        var toRemove = CurrentChapterStats.Rooms.Keys.Where(debugName => debugName != currentRoom.DebugRoomName).ToList();
