...
namespace Celeste.Mod.ConsistencyTracker.Stats {
    ...
    public class AverageLastRunsStat : Stat {
        ...
        public override bool ContainsIdentificator(string format) {
            ...
            return Regex.IsMatch(format, ChapterAverageRunDistanceSessionOverX) ||
                   Regex.IsMatch(format, ChapterLastRunDistanceOverX);
        }
        ...
        public override string FormatStat(PathInfo chapterPath, ChapterStats chapterStats, string format) {
            ...
            foreach (Match match in matches) {
                for (var i = 1; i < match.Groups.Count; i++) {
                    ...
                    try {
                        ...
                        if (invalidFormats.ContainsKey(
                                $"{{chapter:averageRunDistanceSession#{match.Groups[i].Value}}}"))
                            continue;
                        invalidFormats.Add($"{{chapter:averageRunDistanceSession#{match.Groups[i].Value}}}",
                            $"<Invalid run count value: {match.Groups[i].Value}>");
                    } catch (ArgumentException) {
                        if (invalidFormats.ContainsKey(
                                $"{{chapter:averageRunDistanceSession#{match.Groups[i].Value}}}"))
                            continue;
                        invalidFormats.Add($"{{chapter:averageRunDistanceSession#{match.Groups[i].Value}}}",
                            $"<Run count value must be 1 or greater: {match.Groups[i].Value}>");
                    } catch (Exception) {
                        if (invalidFormats.ContainsKey(
                                $"{{chapter:averageRunDistanceSession#{match.Groups[i].Value}}}"))
                            continue;
                ...
                for (var i = 1; i < match.Groups.Count; i++) {
                    ...
                    try {
                        ...
                        if (invalidFormats.ContainsKey($"{{chapter:lastRunDistance#{match.Groups[i].Value}}}"))
                            continue;
                        invalidFormats.Add($"{{chapter:lastRunDistance#{match.Groups[i].Value}}}",
                            $"<Invalid run count value: {match.Groups[i].Value}>");
                    } catch (ArgumentException) {
                        if (invalidFormats.ContainsKey($"{{chapter:lastRunDistance#{match.Groups[i].Value}}}"))
                            continue;
            ...
            var toSet =
                (from nr in runCountsToFormatSession.Keys
                 let formatted = runCountsToFormatSession[nr]
                 where formatted == null
                 select nr).ToDictionary(nr => nr, nr => $"{StatManager.FormatDouble(leftOverAvg)}");
            ...
            format = format.Replace(ChapterAverageRunDistanceSession,
        ...
        public override List<KeyValuePair<string, string>> GetPlaceholderExplanations() {
            return new List<KeyValuePair<string, string>>()
            {
                ...
                new KeyValuePair<string, string>("{chapter:lastRunDistance#X}",
                    "The room number of last run nr. X (e.g. {chapter:lastRunDistance#1} would give you the room number of the most recent golden run, #2 of the 2nd most recent, ...)"),
            };
        }
        ...
        public override List<StatFormat> GetStatExamples() {
            ...
            {
                new StatFormat("avg-run-distance",
                    $"Avg. run distance: {ChapterAverageRunDistance}/{LiveProgressStat.ChapterRoomCount}" +
