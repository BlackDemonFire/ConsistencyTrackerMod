...
namespace Celeste.Mod.ConsistencyTracker.Stats
{
    ...
    public class CurrentRunPbStat : Stat
    {
        ...
        public static string RunCurrentPbStatusPercentSession =
            "{run:currentPbStatusPercentSession}";
        ...
        public override List<KeyValuePair<string, string>> GetPlaceholderExplanations()
        {
            return new List<KeyValuePair<string, string>>()
            {
                new KeyValuePair<string, string>(
                    RunCurrentPbStatus,
                    "Rank of the current run vs. all other runs, e.g. 3rd best run, or PB run"
                ),
                new KeyValuePair<string, string>(
                    RunCurrentPbStatusPercent,
                    "Percentage of many runs the current tops, e.g. The current run is better than 85% of all runs"
                ),
                new KeyValuePair<string, string>(
                    RunCurrentPbStatusSession,
                    $"Same as {RunCurrentPbStatus}, but only for the current session"
                ),
                new KeyValuePair<string, string>(
                    RunCurrentPbStatusPercentSession,
                    $"Same as {RunCurrentPbStatusPercent}, but only for the current session"
                ),
                new KeyValuePair<string, string>(
                    RunTopXPercent,
                    $"Opposite percentage of {RunCurrentPbStatusPercent}, e.g. The current run is in the top 15% of all runs"
                ),
                new KeyValuePair<string, string>(
                    RunTopXPercentSession,
                    $"Opposite percentage of {RunCurrentPbStatusPercentSession}"
                ),
            };
        }
