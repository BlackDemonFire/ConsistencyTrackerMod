    ...
    public class SuccessRateColorsStat : Stat
    {
        ...
        public static List<string> IDs =
            new()
            {
                ...
                ColorLightGreen,
                CheckpointColorRed,
                CheckpointColorYellow,
                CheckpointColorGreen,
                CheckpointColorLightGreen,
                ChapterListColorRed,
                CheckpointListColorRed,
            };
        ...
        )
        {
            ...
            List<string> listColorRedChapter = new();
            List<string> listColorRedCheckpoint = new();
            ...
            foreach (CheckpointInfo cpInfo in chapterPath.Checkpoints)
            {
                ...
                List<string> tempColorListCp = new();
            ...
            else
            {
                ...
                format = format.Replace(
                    CheckpointListColorRed,
                    $"{string.Join(", ", listColorRedCheckpoint)}"
                );
            }

            format = format.Replace(
                ChapterListColorRed,
                $"{string.Join(", ", listColorRedChapter)}"
            );
        ...
        public override List<KeyValuePair<string, string>> GetPlaceholderExplanations()
        {
            return new List<KeyValuePair<string, string>>()
            {
                new KeyValuePair<string, string>(
                    ColorRed,
                    "Count of red rooms (success rate <50%)"
                ),
                new KeyValuePair<string, string>(
                    ColorYellow,
                    "Count of yellow rooms (success rate 50%-80%)"
                ),
                new KeyValuePair<string, string>(
                    ColorGreen,
                    "Count of green rooms (success rate 80%-95%)"
                ),
                new KeyValuePair<string, string>(
                    ColorLightGreen,
                    "Count of light green rooms (success rate 95%-100%)"
                ),
                new KeyValuePair<string, string>(
                    CheckpointColorRed,
                    "Count of red rooms in the current checkpoint"
                ),
                new KeyValuePair<string, string>(
                    CheckpointColorYellow,
                    "Count of yellow rooms in the current checkpoint"
                ),
                new KeyValuePair<string, string>(
                    CheckpointColorGreen,
                    "Count of green rooms in the current checkpoint"
                ),
                new KeyValuePair<string, string>(
                    CheckpointColorLightGreen,
                    "Count of light green rooms in the current checkpoint"
                ),
                new KeyValuePair<string, string>(
                    ChapterListColorRed,
                    "Lists all red rooms in the chapter by name"
                ),
                new KeyValuePair<string, string>(
                    CheckpointListColorRed,
                    "Lists all red rooms in the current checkpoint by name"
                ),
            };
        }
        ...
        public override List<StatFormat> GetStatExamples()
        {
            return new List<StatFormat>()
            {
                new StatFormat(
                    "color-tracker",
                    $"Reds: {ColorRed}, Yellows: {ColorYellow}, Greens: {ColorGreen}, Light-Greens: {ColorLightGreen}"
                ),
                new StatFormat(
                    "color-tracker-cp",
                    $"Checkpoint: Reds: {CheckpointColorRed}, Yellows: {CheckpointColorYellow}, Greens: {CheckpointColorGreen}, Light-Greens: {CheckpointColorLightGreen}"
                ),
                new StatFormat(
                    "red-rooms-list",
                    $"All red rooms: {ChapterListColorRed}\\nIn CP: {CheckpointListColorRed}"
                )
            };
        }
    }
