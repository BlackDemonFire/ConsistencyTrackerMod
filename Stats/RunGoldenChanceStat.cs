...
public class RunGoldenChanceStat : Stat {
    ...
    public static List<string> IDs = new() { RunGoldenChanceFromStart, RunGoldenChanceToEnd };
    ...
    public override string FormatStat(
        ...
        foreach (var cpInfo in chapterPath.Checkpoints) {
            foreach (var rInfo in cpInfo.Rooms) {
                ...
                var successRate = rStats.AverageSuccessOverN(StatManager.AttemptCount);
        ...
        format = format.Replace(
            RunGoldenChanceFromStart,
            StatManager.FormatPercentage(runFromStart)
        );
    ...
    public override List<KeyValuePair<string, string>> GetPlaceholderExplanations() {
        return new List<KeyValuePair<string, string>>()
        {
            new KeyValuePair<string, string>(
                RunGoldenChanceFromStart,
                "The chance for a run to reach the current room from the start"
            ),
            new KeyValuePair<string, string>(
                RunGoldenChanceToEnd,
                "The chance for a run to get to the end from the current room"
            ),
        };
    }
    ...
    public override List<StatFormat> GetStatExamples() {
        return new List<StatFormat>()
        {
            new StatFormat(
                "run-golden-chance",
                $"Start->Room: {RunGoldenChanceFromStart}\\nRoom->End: {RunGoldenChanceToEnd}"
            )
        };
    }
}