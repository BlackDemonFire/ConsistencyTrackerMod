...
public class LiveProgressStat : Stat {
    ...
    public static List<string> IDs = new() {
            ChapterRoomCount,
            CheckpointRoomCount,
            RunChapterProgressNumber,
            RunCheckpointProgressNumber,
            RunChapterProgressPercent,
            RunCheckpointProgressPercent
        };
    ...
    public override string FormatStat(
        ...
        var chapterRoomCount = chapterPath.RoomCount;
        ...
        var checkpointRoomCount = chapterPath.CurrentRoom.Checkpoint.Rooms.Count;

        var roomNumberInChapter = chapterPath.CurrentRoom.RoomNumberInChapter;
        var roomNumberInCP = chapterPath.CurrentRoom.RoomNumberInCP;
        ...
        var decimalPlaces = ConsistencyTrackerModule
            .Instance
            .ModSettings
            .LiveDataDecimalPlaces;
        var chapterProgressPercent = Math.Round(
            ((double)roomNumberInChapter / chapterRoomCount) * 100,
            decimalPlaces
        );
        var checkpointProgressPercent = Math.Round(
            ((double)roomNumberInCP / checkpointRoomCount) * 100,
            decimalPlaces
        );
    ...
    public override List<KeyValuePair<string, string>> GetPlaceholderExplanations() {
        ...
        {
            new(RunChapterProgressNumber,"Number of the room within the entire chapter"),
            new(ChapterRoomCount, "Count of rooms in the chapter"),
            new(RunChapterProgressPercent, "Percent completion of the chapter given the current room"),
            new(RunCheckpointProgressNumber, "Number of the room within the current checkpoint"),
            new(CheckpointRoomCount, "Count of rooms in the current checkpoint"),
            new(RunCheckpointProgressPercent, "Percent completion of the current checkpoint given the current room"),
        };
    }
    ...
    public override List<StatFormat> GetStatExamples() {
        return new List<StatFormat>()
        {
            new(
                "live-progress",
                $"Room {RunChapterProgressNumber}/{ChapterRoomCount} ({RunChapterProgressPercent})"
