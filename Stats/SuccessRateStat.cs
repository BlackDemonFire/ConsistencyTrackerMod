...
namespace Celeste.Mod.ConsistencyTracker.Stats {
    ...
    public class SuccessRateStat : Stat {
        ...
        public override string FormatStat(
            ...
            format = format.Replace(RoomSuccessRate,
                $"{StatManager.FormatPercentage(chapterStats.CurrentRoom.AverageSuccessOverN(attemptCount))}");
            ...
            format = format.Replace(ChapterSuccessRate,
                $"{StatManager.FormatPercentage(chapterPath.Stats.SuccessRate)}");

            var foundRoom = false;

            //Walk the path
            foreach (var cpInfo in chapterPath.Checkpoints.Where(cpInfo =>
                         cpInfo.Rooms.Any(rInfo => rInfo.DebugRoomName == chapterStats.CurrentRoom.DebugRoomName))) {
                format = format.Replace(CheckpointSuccessRate,
                    $"{StatManager.FormatPercentage(cpInfo.Stats.SuccessRate)}");
                foundRoom = true;
            }
        ...
        public override List<KeyValuePair<string, string>> GetPlaceholderExplanations() {
            ...
            {
                new KeyValuePair<string, string>(RoomSuccessRate,
                    "Current room's success rate over last X attempts (count of successful attempts / total attempts)"),
                ...
                new KeyValuePair<string, string>(RoomSuccesses,
                    "Count of successful room clears within the last X attempts (X is the attempt count configured in Mod Options)"),
        ...
        public override List<StatFormat> GetStatExamples() {
            ...
            {
                new StatFormat("success-rate",
                    $"Room SR: {RoomSuccessRate} ({RoomSuccesses}/{RoomAttempts}) | CP: {CheckpointSuccessRate} | Total: {ChapterSuccessRate}")
            };
        }
    }
}