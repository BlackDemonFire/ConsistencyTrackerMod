    ...
    public class PersonalBestStat : Stat
    {
        ...
        )
        {
            ...
            for (int cpIndex = chapterPath.Checkpoints.Count - 1; cpIndex >= 0; cpIndex--)
            {
                ...
                for (int roomIndex = cpInfo.Rooms.Count - 1; roomIndex >= 0; roomIndex--)
                {
                    ...
                    int goldenDeathsSession = chapterStats
        ...
        public override List<KeyValuePair<string, string>> GetPlaceholderExplanations()
        {
            return new List<KeyValuePair<string, string>>()
            {
                new KeyValuePair<string, string>(
                    PBBest,
                    "Name of the PB room + the death count if it's greater than 1"
                ),
                new KeyValuePair<string, string>(
                    PBBestSession,
                    $"Same as {PBBest}, but only runs in the current session"
                ),
                new KeyValuePair<string, string>(
                    "{pb:best#<num>}",
                    $"Same as {PBBest}, but for the <num>'s best run. {{pb:best#1}} is equivalent to {PBBest}"
                ),
                new KeyValuePair<string, string>(
                    "{pb:bestSession#<num>}",
                    $"Same as {PBBest} for current session, but for the <num>'s best run. {{pb:bestSession#1}} is equivalent to {PBBestSession}"
                ),
                new KeyValuePair<string, string>(
                    PBBestRoomNumber,
                    "Number of the PB room in the chapter"
                ),
                new KeyValuePair<string, string>(
                    PBBestRoomNumberSession,
                    $"Same as {PBBestRoomNumber}, but only runs in the current session"
                ),
                new KeyValuePair<string, string>(
                    "{pb:bestRoomNumber#<num>}",
                    $"Same as {PBBestRoomNumber}, but for the <num>'s best run. {{pb:bestRoomNumber#1}} is equivalent to {PBBestRoomNumber}"
                ),
                new KeyValuePair<string, string>(
                    "{pb:bestRoomNumberSession#<num>}",
                    $"Same as {PBBestRoomNumber} for current session, but for the <num>'s best run. {{pb:bestRoomNumberSession#1}} is equivalent to {PBBestRoomNumberSession}"
                ),
            };
        }
        ...
        public override List<StatFormat> GetStatExamples()
        {
            return new List<StatFormat>()
            {
                new StatFormat(
                    "pbs",
                    $"Best runs: {PBBest} | {{pb:best#2}} | {{pb:best#3}} | {{pb:best#4}} | {{pb:best#5}}"
                ),
                new StatFormat(
                    "pbs-session",
                ...
                new StatFormat(
                    "pb-only",
                    $"PB: {PBBest} ({PBBestRoomNumber}/{LiveProgressStat.ChapterRoomCount})"
                ),
            };
        }
    }
