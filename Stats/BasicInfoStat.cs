    ...
    public class BasicInfoStat : Stat
    {
        ...
        public static List<string> IDs =
            new()
            {
                ...
                CheckpointName,
                CheckpointAbbreviation,
                CheckpointGoldenDeaths,
                CheckpointGoldenDeathsSession,
                CheckpointGoldenChance,
                /*ChapterName, ChapterDebugName,*/ChapterGoldenDeaths,
                ChapterGoldenDeathsSession,
                ChapterGoldenChance,
                //CampaignName
            };
        ...
        )
        {
            ...
            format = format.Replace(
                ChapterGoldenDeathsSession,
                $"{chapterPath.Stats.GoldenBerryDeathsSession}"
            );
            format = format.Replace(
                ChapterGoldenChance,
        ...
        public override List<KeyValuePair<string, string>> GetPlaceholderExplanations()
        {
            return new List<KeyValuePair<string, string>>()
            {
                ...
                new KeyValuePair<string, string>(
                    ChapterDebugName,
                    "[DEV] Debug name of the chapter"
                ),
                new KeyValuePair<string, string>(ChapterSID, "[DEV] SID of chapter"),
                new KeyValuePair<string, string>(
                    ChapterSanitizedSID,
                    "[DEV] Dialog sanitized SID of chapter"
                ),
                new KeyValuePair<string, string>(
                    ChapterGoldenDeaths,
                    "Golden Deaths in the chapter"
                ),
                new KeyValuePair<string, string>(
                    ChapterGoldenDeathsSession,
                    "Golden Deaths in the chapter in the current session"
                ),
                new KeyValuePair<string, string>(
                    ChapterGoldenChance,
                    "Golden Chance of the chapter"
                ),
                new KeyValuePair<string, string>(CheckpointName, "Name of the current checkpoint"),
                new KeyValuePair<string, string>(
                    CheckpointAbbreviation,
                    "Abbreviation of the current checkpoint's name"
                ),
                new KeyValuePair<string, string>(
                    CheckpointGoldenDeaths,
                    "Golden Deaths in the current checkpoint"
                ),
                new KeyValuePair<string, string>(
                    CheckpointGoldenDeathsSession,
                    "Golden Deaths in the current checkpoint in the current session"
                ),
                new KeyValuePair<string, string>(
                    CheckpointGoldenChance,
                    "Golden Chance of the current checkpoint"
                ),
                new KeyValuePair<string, string>(
                    RoomName,
                    "Name of the room. Display format can be changed via Mod Options -> Consistency Tracker -> Live Data -> Room Name Format"
                ),
                new KeyValuePair<string, string>(RoomDebugName, "Debug name of the current room"),
                new KeyValuePair<string, string>(
                    RoomGoldenDeaths,
                    "Golden Deaths in the current room"
                ),
                new KeyValuePair<string, string>(
                    RoomGoldenDeathsSession,
                    "Golden Deaths in the current room in the current session"
                ),
                new KeyValuePair<string, string>(
                    PlayerHoldingGolden,
                    "Whether the player is holding a golden berry"
                ),
                new KeyValuePair<string, string>(
                    ModTrackingPaused,
                    "Whether death tracking is currently paused"
                ),
                new KeyValuePair<string, string>(
                    ModRecordingPath,
                    "Whether the path is currently being recorded"
                ),
                new KeyValuePair<string, string>(ModModVersion, "Current version of the mod"),
                new KeyValuePair<string, string>(
                    ModOverlayVersion,
                    "Most recent version of the overlay"
                ),
            };
        }
